...
app.get('/fetchDealer/:id', async (req, res) => {
    ...
    try {
        // Query by the 'id' field instead of MongoDB's default '_id'
        const document = await Dealerships.findOne({ id: parseInt(id) }); // Assuming 'id' is an integer
        if (document) {
            res.json(document);
        } else {
            console.log('Dealer not found for ID:', id);
            res.status(404).json({ error: 'Dealer not found' });
        }
    } catch (error) {
